...
class Ion_auth_model extends CI_Model
{
	...
	public function admin_check($username = '')
	{
		...
		return $this->db->where('username', $username)
		                ->count_all_results($this->tables['admin']) > 0;
	}
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		if ($this->identity_column != 'username')
		{
			...
			for($i = 0; $this->admin_check($username); $i++)
			{
				if($i > 0)
				{
	...
	// @Zihao Wang
	// @customize for cegHypermarket admin
	public function admin_register($username, $password, $email, $additional_data = array())
	{
		$this->trigger_events('pre_register');

		$manual_activation = $this->config->item('manual_activation', 'ion_auth');

		if ($this->identity_column == 'email' && $this->admin_email_check($email))
		{
			$this->set_error('account_creation_duplicate_email');
			return FALSE;
		}
		elseif ($this->identity_column == 'username' && $this->admin_check($username))
		{
			$this->set_error('account_creation_duplicate_username');
			return FALSE;
		}
		...
		//filter out any data passed that doesnt have a matching column in the users table
		//and merge the set user data and the additional data
		$user_data = array_merge($this->_filter_data($this->tables['admin'], $additional_data), $data);

		$this->trigger_events('extra_set');

		$this->db->insert($this->tables['admin'], $user_data);
	...
	// @Zihao Wang
	// @customize for cegHypermarket admin
	public function admin_login($identity, $password, $remember=FALSE)
	{
		$this->trigger_events('pre_login');
		...
		$query = $this->db->select($this->identity_column . ', email, a_id, password, last_name, first_name, position, store_id')
		                  ->where($this->identity_column, $this->db->escape_str($identity))
		                  ->limit(1)
		                  ->get($this->tables['admin']);
		...
		if ($query->num_rows() === 1)
		{
			...
			$password = $this->hash_password_db($user->a_id, $password);
			...
			if ($password === TRUE)
			{
				...
				$session_data = array(
				    ...
				    'username'             => $user->username,
				    'email'                => $user->email,
				    'a_id'             	   => $user->a_id, //everyone likes to overwrite id so we'll use user_id
	...
	{
		$this->trigger_events('admin');

		//if no id was passed use the current users id
		$id || $id = $this->session->userdata('a_id');

		$this->limit(1);
		$this->where($this->tables['admin'].'.a_id', $id);
